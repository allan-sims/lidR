...
streamLAS.character = function(x, ofile, select = "*", filter = "", filter_wkt = "")
{
  ...
  if (utils::packageVersion("rlas") >= "1.8.0") {
    data <- rlas::read_and_write.las(ifiles, ofile, select, filter, geom, nthreads = nthreads)
  } else {
    data <- rlas::read_and_write.las(ifiles, ofile, select, filter, filter_wkt, nthreads = nthreads)
  }
